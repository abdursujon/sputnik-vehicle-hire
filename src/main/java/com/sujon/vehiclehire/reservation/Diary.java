...
/**
 * A class main.Diary that represents a "diary" of vehicle reservations.
 * <p>
...
public class Diary {
    ...
    /**
     * Constructor for objects of class main.Diary
     */
    public Diary() {
     ...
     *
     * @parameter vehicleReservation, of type main.VehicleReservation
     */
    public void addReservation(BookingDetails vehicleReservation) {
    ...
    public void printEntries(Date startDate, Date endDate) {
        if (DateUtil.daysBetween(startDate, endDate) < 0) {
            ...
            System.out.println("\n\nmain.Diary: Reservations for period " + DateUtil.convertDateToShortString(startDate)
     ...
     * @parameter vehicleReservation, of type main.VehicleReservation
     */
    public void deleteReservation(BookingDetails vehicleReservation) {
        Date date = vehicleReservation.getStartDate();
        for (int day = 1; day <= vehicleReservation.getNoOfDays(); day++) {
            DayInDiary dayInDiary = dayInDiaryMap.get(date);
     ...
     */
    public BookingDetails[] getReservations(Date date) {
        DayInDiary dayinDiary = dayInDiaryMap.get(date);
        if (dayinDiary == null)
            return null;
        else
    ...
    // inner class, only needed in the main.Diary class
    private class DayInDiary {
        ...
        private BookingDetails[] getReservations() {
            int noOfEntries = daysEntries.size();
            BookingDetails[] vehicleReservations = new BookingDetails[noOfEntries];
            for (int i = 0; i < noOfEntries; i++)
                vehicleReservations[i] = daysEntries.get(i).getReservation();
            return vehicleReservations;
        }

        private void addEntry(BookingDetails vehicleReservation, int dayNo) {
            daysEntries.add(new Entry(vehicleReservation, dayNo));
        }

        private void deleteEntry(BookingDetails vehicleReservation) {
        ...
        private void printEntries() {
            ...
            if (size > 0) {
                ...
                for (Entry entry : daysEntries) {
                    BookingDetails vehicleReservation = entry.getReservation();
        ...
        private class Entry {
            private BookingDetails vehicleReservation;
            private int reservationDay;  // e.g. second day is day 2 of 4 for a reservation spanning 4 days

            private Entry(BookingDetails vehicleReservation, int reservationDay) {
                this.vehicleReservation = vehicleReservation;
                this.reservationDay = reservationDay;
            }

            private BookingDetails getReservation() {
